...
//Init is called during chaincode instantitiation to initiliaze any data
...
func (t *SimpleChaincode) createLot(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	lot := Lot{sgtin, productID, quantity, unit, organisationID, status, precessorLotIDs, successorLotIds, storageSpaceID}
...
func (t *SimpleChaincode) createProduct(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	product := Product{productID, args[1], args[2]}
	productAsBytes, _ := json.Marshal(product)
	stub.PutState("PRO"+args[0], productAsBytes)
...
func (t *SimpleChaincode) initProducts(stub shim.ChaincodeStubInterface) pb.Response {
	...
	products := []Product{
		Product{123, "Milk", "Best milk ever produced in the World!"},
		Product{234, "Cheese", "Cheesiest cheese in the universum!"},
	}
