...
SIGNATURE_POLICY_ALL="\"OR('DeoniMSP.member','TuxerMSP.member','BrangusMSP.member','SalersMSP.member','PinzgauerMSP.member','AuthorityMSP.member')\""
SIGNATURE_POLICY_MINIMAL="\"OR('DeoniMSP.member','BrangusMSP.member','SalersMSP.member')\""
LARGE_NETWORK_MEMBERS=( cli.deoni.de cli.brangus.de cli.pinzgauer.de cli.tuxer.de cli.salers.de cli.authority.de)
...
LARGE_PEER_ADDRESSES=(peer0.deoni.de:7051 peer0.brangus.de:7051 peer0.pinzgauer.de:7051 peer0.tuxer.de:7051 peer0.salers.de:7051 peer0.authority.de:7051)
SMALL_PEER_ADDRESSES=(peer0.deoni.de:7051 peer0.brangus.de:7051 peer0.salers.de:7051)
LARGE_PEER_CERTFILES=( /etc/hyperledger/msp/users/admin/tls/ca.crt ${TLS_CA_CERTS_FOLDER}/tlsca.brangus.de-cert.pem ${TLS_CA_CERTS_FOLDER}/tlsca.pinzgauer.de-cert.pem ${TLS_CA_CERTS_FOLDER}/tlsca.tuxer.de-cert.pem ${TLS_CA_CERTS_FOLDER}/tlsca.salers.de-cert.pem ${TLS_CA_CERTS_FOLDER}/tlsca.authority.de-cert.pem)
...
then
  ...
  collections_filename=$LARGE_COLLECTIONS_FILENAME
  ...
  collections_filename=$SMALL_COLLECTIONS_FILENAME
...
do
 ...
 CC_PACKAGE_ID=$(docker exec cli.deoni.de bash -c "peer lifecycle chaincode queryinstalled | grep Label| sort | tr -s ' '| cut -d ' ' -f 3 | cut -d , -f 1 | tail -n1")
  ...
  echo "Approve on '$i'"
  docker exec $i bash -c "peer lifecycle chaincode approveformyorg -o orderer.unibw.de:7050  --channelID '$CHANNEL' --name '$CCNAME' --version '$CCVERSION' --package-id '${CC_PACKAGE_ID}' --sequence '$SEQUENCE' --collections-config /opt/gopath/src/github.com/'$CCNAME'/$collections_filename --signature-policy $SIGNATURE_POLICY  --tls --cafile /etc/hyperledger/msp/users/admin/tls/tlsca.unibw.de-cert.pem"
done
